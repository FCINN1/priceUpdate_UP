...
async function main() {
  try {
    ...
    const UP_LIST = await playerSearch([246], 0); // playerSearch(시즌넘버, 최소오버롤)
    let UP_RESULTS = await playerPriceValue(UP_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(UP_RESULTS);
